...
<div class="w-fit">
    <button type="button" class="w-min-[142px] h-[60px] rounded-[30px] group flex items-center justify-center py-5 px-7 space-x-3.5 {{ $stylesButton[$color] ?? $stylesButton['green'] }}">
        <x-icon :icon="$icon" :color="$color"></x-icon>
        <p class="elementText text-nowrap">{{ $buttonText }}</p>
    </button>
</div>
